          ...
          <style>
            * {{
            ...
            .contractor-card {{
              ...
              box-shadow: 0 0 0 1px rgba(255,255,255,0.02) inset, 0 0 22px rgba(0,0,0,0.2);
            }}
            .contractor-card h3 {{
              font-size: 22px;
              margin-bottom: 8px;
            ...
            .vetted-badge {{
              ...
              margin-bottom: 12px;
              border-radius: 16px;
              background: linear-gradient(135deg, rgba(13,13,13,0.98) 0%, rgba(43,18,7,0.96) 42%, rgba(111,22,10,0.92) 100%);
              border: 1px solid rgba(212,175,55,0.5);
              box-shadow: 0 0 0 1px rgba(255,255,255,0.03) inset, 0 10px 24px rgba(0,0,0,0.24);
              color: #f8e5a1;
            }}
            .vetted-badge .badge-icon {{
              ...
              background: linear-gradient(180deg, #f0d37a 0%, #d4af37 58%, #a97816 100%);
              color: #200a02;
              font-weight: 900;
              font-size: 14px;
            ...
            .vetted-badge .badge-copy {{
              display: flex;
              flex-direction: column;
              gap: 2px;
              line-height: 1.05;
            }}
            .vetted-badge .badge-top {{
              font-size: 10px;
              font-weight: 800;
            ...
            .vetted-badge.homepage {{
              padding: 12px 16px;
              border-radius: 18px;
            }}
            .vetted-badge.homepage .badge-icon {{
              width: 52px;
              min-width: 52px;
              height: 60px;
              font-size: 17px;
            }}
            .vetted-badge.homepage .badge-top {{
              font-size: 11px;
            }}
            .vetted-badge.homepage .badge-main {{
              font-size: 16px;
            }}
            .btn {{
...
def format_contractor_card(contractor: dict):
    ...
    services = [s.strip() for s in split_services(str(service_raw)) if s.strip()]
    primary_service = esc(services[0].title() if services else "General Contractor")
    service_count = len(services)
    city = esc(contractor.get("city"))
    state = esc(contractor.get("state"))
    approved = contractor.get("approved") is True
    company_display = company_name or full_name or "Contractor"
    owner_line = f"<div class="contractor-mini">{full_name}</div>" if full_name and full_name != company_display else ""
    location_line = ", ".join([part for part in [city, state] if part]).strip(", ") or "Service area available"
    extra_service_chip = f'<span class="meta-chip">+{service_count - 1} more trade{"s" if service_count - 1 != 1 else ""}</span>' if service_count > 1 else ""
    vetted_badge = ""
    if approved:
        vetted_badge = """
        <div class="vetted-badge">
          <span class="badge-icon">VF</span>
          <span class="badge-copy"><span class="badge-top">VaultForge Approved</span><span class="badge-main">Vetted Contractor</span></span>
        </div>
        """
    ...
    <div class="card contractor-card">
      ...
      <div>
        ...
        <div class="contractor-subline">{primary_service}</div>
        {owner_line}
        <div class="meta-chip-row">
          <span class="meta-chip">{location_line}</span>
          {extra_service_chip}
        </div>
        <p class="contractor-mini">One homeowner. One chosen contractor. Direct request flow.</p>
      </div>
...
def contractors(service: str = ""):
    ...
    <div class="hero">
      <h1>{esc(heading)}</h1>
      <p class="muted">Browse approved contractors in a compact window view and choose the one you want to request.</p>
    </div>
    <div class="contractor-window-grid">
      {cards}
    </div>
    """
...
def contractor_dashboard(contractor_id: str, token: str = ""):
    ...
    <div class="hero premium-panel">
      ...
      {('<div class="vetted-badge subtle"><span class="badge-icon">VF</span><span class="badge-copy"><span class="badge-top">VaultForge Approved</span><span class="badge-main">Vetted Contractor</span></span></div>' if contractor.get("approved") is True else '')}
