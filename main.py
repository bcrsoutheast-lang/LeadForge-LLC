...
app = FastAPI(title="LeadForge API", version="1.1.0")
...
FRONTEND_BASE_URL = os.getenv("FRONTEND_BASE_URL", "https://lead-forge-frontend-new.vercel.app").rstrip("/")
...
def create_unlock_record_if_missing(
    ...
    try:
        ...
        if not unlock_row:
            raise HTTPException(status_code=500, detail="Unlock record was not returned after insert")
        return unlock_row
        ...
        message = str(e)
        if "lead_unlocks" in message.lower() or "relation" in message.lower():
            raise HTTPException(status_code=500, detail="lead_unlocks table is missing in Supabase.")
...
def fulfill_checkout_session(session: Any) -> Dict[str, Any]:
    ...
    if not lead_id:
        raise HTTPException(status_code=400, detail="Checkout session missing lead_id metadata")
...
def verify_checkout_session(session_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve Stripe checkout session: {str(e)}")
...
def create_contractor(contractor: ContractorCreate):
    payload = {
        "company_name": contractor.company_name.strip() if contractor.company_name else None,
        "contact_name": contractor.contact_name.strip() if contractor.contact_name else None,
        "first_name": contractor.first_name.strip() if contractor.first_name else None,
        "last_name": contractor.last_name.strip() if contractor.last_name else None,
        "business_name": contractor.business_name.strip() if contractor.business_name else None,
        "trade": contractor.trade.strip() if contractor.trade else None,
        "experience_years": contractor.experience_years.strip() if contractor.experience_years else None,
        "service_area": contractor.service_area.strip() if contractor.service_area else None,
        "phone": contractor.phone.strip(),
        "email": contractor.email.strip(),
        "license_number": contractor.license_number.strip() if contractor.license_number else None,
        "licensed_insured": contractor.licensed_insured.strip() if contractor.licensed_insured else None,
        "insured": contractor.insured,
        "notes": contractor.notes.strip() if contractor.notes else None,
    ...
    try:
        ...
        for contractor in contractors:
            ...
            <tr>
                ...
                <td>{contractor.get('business_name', contractor.get('company_name', ''))}</td>
                <td>{contractor.get('first_name', '')} {contractor.get('last_name', '')}</td>
                <td>{contractor.get('phone', '')}</td>
                <td>{contractor.get('email', '')}</td>
                <td>{contractor.get('trade', contractor.get('services_offered', ''))}</td>
                <td>{contractor.get('service_area', '')}</td>
                <td>{contractor.get('license_number', contractor.get('licensed_insured', ''))}</td>
