...
def brand_html() -> str:
    ...
    <a class="brand-wrap" href="/">
      <img src="/static/IMG_2255.png" alt="VaultForge Logo" class="brand-logo" onerror="this.style.display='none'">
...
def safe_insert_contractor(payload: dict) -> tuple[bool, str]:
    ...
    attempts = [
        ...
        {k: v for k, v in payload.items() if k not in {"business_description"}},
        {
            ...
            if k in {"id", "full_name", "company_name", "phone", "email", "service", "city", "state", "zip", "approved", "created_at"}
        },
        {
            k: v for k, v in payload.items()
            if k in {"full_name", "company_name", "phone", "email", "service", "city", "state", "zip", "approved", "created_at"}
        },
    ]
...
def safe_insert_lead(payload: dict) -> tuple[bool, str]:
    ...
    attempts = [
        ...
        {
            k: v for k, v in payload.items()
            if k in {"id", "contractor_id", "owner_name", "phone", "email", "service", "project_details", "city", "state", "zip", "created_at"}
        },
        {
            k: v for k, v in payload.items()
            if k in {"contractor_id", "owner_name", "phone", "email", "service", "project_details", "city", "state", "zip", "created_at"}
        },
        {
            k: v for k, v in payload.items()
            if k in {"contractor_id", "owner_name", "phone", "service", "project_details", "city", "state"}
        },
    ]
...
def format_copy_line(lead: dict) -> str:
...
def homepage():
    ...
    <div class="grid grid-2" style="margin-top:22px;">
      ...
      <div class="card">
        <h2>Why homeowners use VaultForge</h2>
        <ul class="clean-list mini">
    ...
    <div class="card" style="margin-top:22px;">
      ...
      <p class="mini">
        VaultForge is meant to feel legitimate. Homeowners should feel like they are choosing from contractors
...
def admin_dashboard(request: Request):
    ...
    leads_by_contractor: dict[str, list[dict]] = {}
    orphan_leads: list[dict] = []
