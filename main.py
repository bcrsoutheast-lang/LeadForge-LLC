...
def stripe_success(session_id: str = ""):
    ...
    if session.get("payment_status") == "paid" and lead_id:
        unlock_lead(lead_id)
        lead = fetch_lead_by_id(lead_id)

        return layout("Unlocked", f"""
        {topbar()}
        <h1>Lead Unlocked</h1>
        <p>{esc(lead.get("owner_name") if lead else "")}</p>
        <a href="/contractor/{contractor_id}">Dashboard</a>
        """)

    return error_page("Payment Error", "Not completed")


# 🔥 NEW WEBHOOK (IMPORTANT)
