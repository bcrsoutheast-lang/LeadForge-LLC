...
print("MULTI-SERVICE CONTRACTOR VERSION")
...
def normalize_services(values) -> List[str]:
    if not values:
        return []
...
def service_matches(contractor_services, selected_service: str) -> bool:
    selected_value = normalize_service(selected_service)

    if not selected_value:
        return False

    contractor_values = normalize_services(contractor_services)

    if not contractor_values:
        return False
    ...
    for contractor_value in contractor_values:
...
def submit_contractor(
    ...
    print("NEW CONTRACTOR SUBMITTED:", contractor)
...
def contractors(request: Request, service: str = None):
    ...
    if service:
        data = [
            ...
            if service_matches(
                contractor.get("services", contractor.get("service", "")),
                service
            )
        ]
