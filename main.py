...
def contractor_public_card(c: dict) -> str:
    ...
    business_name = c.get("business_name", "Unnamed Business")
    service = c.get("service", "")
    city = c.get("city", "")
    state = c.get("state", "")
    description = c.get("description", "") or "No description yet."
...
def admin_login_page(error: str = "") -> HTMLResponse:
    error_html = f'<p style="color:#b91c1c;font-weight:700;">{error}</p>' if error else ""
    return html_page(
        "Admin Login",
        f"""
        <div class="topbar">
    ...
    for c in contractors:
        ...
        contractor_rows.append(
            ...
            <tr>
                ...
                <td>{c.get("owner_name", "")}</td>
                <td>{c.get("email", "")}</td>
                <td>{c.get("phone", "")}</td>
                <td>{c.get("service", "")}</td>
                <td>{c.get("city", "")}, {c.get("state", "")}</td>
                ...
                <td>
                    ...
                    <a class="btn btn-secondary small" href="/contractor/{contractor_id}">Dashboard</a>
                </td>
            </tr>
            """
        )
    ...
    return html_page(
        ...
        <div class="card">
            ...
            <table>
                ...
                <tbody>
                    {''.join(contractor_rows) if contractor_rows else '<tr><td colspan="8">No contractors found.</td></tr>'}
                </tbody>
            </table>
        </div>
...
def home():
    return html_page(
        ...
        <div class="topbar">
            <div>
                <h1>LeadForge</h1>
                <p class="muted">Browse vetted contractors. Choose one. Submit one exclusive request.</p>
            </div>
            <div>
                <a class="btn btn-secondary" href="/contractors">Browse Contractors</a>
        ...
        <div class="grid">
            <div class="card">
                ...
                <p>Browse approved contractors and send your request directly to the one you choose.</p>
                <a class="btn" href="/contractors">Find a Contractor</a>
            </div>
            <div class="card">
                <h3>For Contractors</h3>
                <p>Receive exclusive homeowner opportunities and unlock full contact details when ready.</p>
                <a class="btn btn-secondary" href="/contractors">See Marketplace</a>
            </div>
        </div>
        """,
    )
...
def request_form(contractor_id: str):
    ...
    business_name = contractor.get("business_name", "Contractor")
...
def approve_contractor(contractor_id: str, request: Request):
    ...
    db = db_required()
    db.table("contractors").update({"approved": True}).eq("id", contractor_id).execute()
    return RedirectResponse(url="/admin", status_code=303)
...
def contractor_dashboard(contractor_id: str):
    ...
    for lead in leads:
        ...
        if unlocked:
            details = f"""
            <p><strong>Homeowner:</strong> {lead.get("owner_name", "")}</p>
            ...
            <p><strong>Location:</strong> {lead.get("city", "")}, {lead.get("state", "")} {lead.get("zip", "")}</p>
            """
            action = ""
        else:
            details = f"""
            ...
            <p><strong>Location:</strong> {lead.get("city", "")}, {lead.get("state", "")} {lead.get("zip", "")}</p>
            """
            action = f'<a class="btn" href="/unlock/{lead_id}">Unlock for $10</a>'
...
# =========================================================
# OPTIONAL CONTRACTOR SIGNUP API
# =========================================================
