...
def admin_dashboard():
    try:
        leads_response = supabase.table("leads").select("*").order("created_at", desc=True).execute()
        contractors_response = supabase.table("contractors").select("*").order("created_at", desc=True).execute()
