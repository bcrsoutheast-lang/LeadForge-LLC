...
def contractor_dashboard(contractor_id: str, token: Optional[str] = None):
    ...
    try:
        ...
        return render_forbidden_page(
            "Private Dashboard",
            "This dashboard requires a valid private contractor link. Contact LeadForge if you need your access link again."
        )
    ...
    for lead in leads:
        ...
        if unlocked:
            ...
            <p><strong>Location:</strong> {lead.get("city", "")}, {lead.get("state", "")} {lead.get("zip", "")}</p>
            """
            action = f'<a class="btn" href="/unlock/{lead_id}?token={valid_token}">Unlock for $10</a>'
