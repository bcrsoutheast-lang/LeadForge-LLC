...
def vetted_badge_html(title: str = "VaultForge Vetted Contractor"):
    return f"""
    <div class="vetted-badge homepage" style="display:inline-flex; margin:0 auto 18px auto;">
      <span class="badge-icon">✓</span>
      <span class="badge-copy"><span class="badge-top">VaultForge Approved</span><span class="badge-main">{esc(title)}</span></span>
    </div>
    """
          ...
          <style>
            * {{
            ...
            .vetted-badge {{
              ...
              margin-bottom: 12px;
              border-radius: 999px;
              background: linear-gradient(135deg, rgba(212,175,55,0.18) 0%, rgba(204,0,0,0.18) 100%);
            ...
            .premium-state-card {{
              ...
              gap: 16px;
              align-items: flex-start;
              background: linear-gradient(135deg, rgba(22,22,22,0.98) 0%, rgba(9,9,9,1) 100%);
              border: 1px solid rgba(212,175,55,0.22);
              box-shadow: 0 0 28px rgba(204,0,0,0.08);
              transition: transform 0.15s ease, border-color 0.15s ease, box-shadow 0.15s ease;
            }}
            .premium-state-card:hover {{
              transform: translateY(-3px);
              border-color: rgba(212,175,55,0.5);
              box-shadow: 0 0 34px rgba(212,175,55,0.09);
            }}
            .state-badge {{
              width: 60px;
              height: 60px;
            ...
            .vetted-badge-window {{
              ...
              padding: 7px 12px;
              border-radius: 999px;
              background: linear-gradient(135deg, rgba(212,175,55,0.18) 0%, rgba(204,0,0,0.16) 100%);
...
def admin_hide_lead(lead_id: str):
    if not supabase:
        return False, "Supabase is not configured."
    try:
        supabase.table("leads").update({"admin_deleted": True}).eq("id", lead_id).execute()
        return True, ""
    except Exception as e:
        return False, str(e)


def contractor_delete_lead_record(lead_id: str, contractor_id: str):
    if not supabase:
        return False, "Supabase is not configured."
    try:
        supabase.table("leads").delete().eq("id", lead_id).eq("contractor_id", contractor_id).execute()
        return True, ""
    except Exception as e:
        return False, str(e)


def delete_contractor_only(contractor_id: str):
    if not supabase:
        return False, "Supabase is not configured."
    try:
...
def format_contractor_card(contractor: dict):
    ...
    service_raw = contractor.get("service") or contractor.get("services") or ""
    services = split_services(str(service_raw))
    primary_service = esc(services[0].title() if services else "Contractor Services")
    service_line = esc(", ".join(s.title() for s in services[:3])) if services else "General Contractor"
    ...
    if approved:
        badge_html = """
        <div class="vetted-badge-window">
          <span class="vetted-shield">🛡️</span>
          <span>VaultForge Vetted</span>
        </div>
        """

    owner_line = f'<div class="contractor-mini">{full_name}</div>' if full_name else ""

    return f"""
    <div class="card contractor-window-card">
      {badge_html}
      <div class="contractor-window-top">
        <div>
          <div class="contractor-overline">{primary_service}</div>
          <h3>{company_name or full_name or "Contractor"}</h3>
          {owner_line}
        </div>
      </div>
      <div class="contractor-location">{city}, {state}</div>
      <div class="contractor-services">{service_line}</div>
      <a class="btn gold contractor-window-btn" href="/request/{contractor_id}">Request Contractor</a>
    </div>
    """
...
def homepage():
    ...
    <div class="hero">
      <div class="hero-kicker gold"><strong>Exclusive Opportunities. No Bidding Wars. No Lead Pile-On.</strong></div>
      <h1><span class="red">Homeowners Choose.</span><br><span class="gold">Contractors Unlock Opportunity.</span></h1>
      <p class="hero-copy">
        VaultForge is built for a cleaner home-services experience. Homeowners choose one vetted contractor.
        Contractors unlock real opportunities instead of paying for crowded shared leads.
      </p>
      <div class="topnav">
        <a class="btn" href="/services">Find a Contractor</a>
        <a class="btn secondary" href="/states">Browse by State</a>
        <a class="btn gold" href="/join-contractor">Join as a Contractor</a>
      </div>
      <div class="hero-premium-bar">
        <div class="hero-premium-item">
          <strong class="gold">One homeowner. One chosen contractor.</strong>
          <div class="mini">No blast-out to a pile of companies.</div>
        </div>
        <div class="hero-premium-item">
          <strong class="gold">Flat $10 unlock</strong>
          <div class="mini">Contractors pay to unlock opportunity, not to chase shared scraps.</div>
        </div>
        <div class="hero-premium-item">
          <strong class="gold">Vetted before listing</strong>
          <div class="mini">Approved contractors stand out immediately to homeowners.</div>
        </div>
      </div>
    </div>
    ...
    <div class="card" style="margin-top:22px; text-align:center;">
      <div class="vetted-badge homepage" style="display:inline-flex; margin:0 auto 18px auto;">
        <span class="badge-icon">✓</span>
        <span class="badge-copy"><span class="badge-top">VaultForge Approved</span><span class="badge-main">Vetted Contractor Badge</span></span>
      </div>
      <h2 class="gold">What the VaultForge Vetted Badge Means</h2>
      <p class="mini" style="max-width:780px; margin:0 auto 16px auto;">
        When homeowners see the <strong>VaultForge Vetted Contractor</strong> badge, it means that contractor
        passed our internal review and was approved to appear in the VaultForge directory.
      </p>
      <div class="grid grid-3" style="margin-top:18px; text-align:left;">
        <div class="card">
          <h3>Reviewed business details</h3>
          <p class="mini">
            We review contractor information before approval so homeowners are not browsing random unknown listings.
          </p>
        </div>
        <div class="card">
          <h3>Licensing and insurance matter</h3>
          <p class="mini">
            Where a trade requires licensing, that matters. Insurance matters too. Approval is meant to reflect a more legitimate professional presence.
          </p>
        </div>
        <div class="card">
          <h3>Approved for homeowner requests</h3>
          <p class="mini">
            Only approved contractors appear in the homeowner directory, and homeowners choose one vetted contractor instead of blasting requests everywhere.
          </p>
        </div>
      </div>
      <p class="mini" style="max-width:760px; margin:18px auto 0 auto; color:#f0d37a;">
        The badge is there to help homeowners feel safer choosing a contractor and to help good contractors stand out fast.
      </p>
    </div>

    <div class="card" style="margin-top:22px;">
      <h2 class="section-title">VaultForge vs. Typical Lead Platforms</h2>
      <p class="mini" style="margin-bottom:18px;">
        Many lead sites use a shared-lead model often associated with platforms such as Angi, HomeAdvisor, and Thumbtack.
        VaultForge is built around a one-homeowner, one-contractor model instead.
      </p>

      <div class="grid grid-2">
        <div class="card">
          <h3 class="gold">VaultForge</h3>
          <ul class="clean-list mini">
            <li>Homeowner chooses one contractor directly.</li>
            <li>Approved contractors stand out with a vetted badge.</li>
            <li>No shared lead pile-on.</li>
            <li>Flat $10 unlock to access the homeowner opportunity.</li>
            <li>Built to reduce bidding wars and wasted spend.</li>
          </ul>
        </div>

        <div class="card">
          <h3>Typical Lead Platforms</h3>
          <ul class="clean-list mini">
            <li>Leads are often sent to multiple contractors.</li>
            <li>Contractors may compete against several companies at once.</li>
            <li>Lead pricing can vary by platform and market.</li>
            <li>Homeowners can get flooded with outreach.</li>
            <li>The experience often feels more like a race than a match.</li>
          </ul>
        </div>
      </div>

      <div class="card" style="margin-top:16px; padding:0; overflow:hidden;">
        <div style="display:grid; grid-template-columns:minmax(160px,1.1fr) 1fr 1fr;">
          <div style="padding:14px 16px; border-right:1px solid #242424; border-bottom:1px solid #242424; font-weight:800; background:#111;">Comparison Point</div>
          <div style="padding:14px 16px; border-right:1px solid #242424; border-bottom:1px solid #242424; font-weight:800; background:rgba(212,175,55,0.08); color:#f0d37a;">VaultForge</div>
          <div style="padding:14px 16px; border-bottom:1px solid #242424; font-weight:800; background:#111;">Typical Lead Sites</div>

          <div style="padding:14px 16px; border-right:1px solid #242424; border-bottom:1px solid #242424;">Who the homeowner contacts</div>
          <div style="padding:14px 16px; border-right:1px solid #242424; border-bottom:1px solid #242424;">One chosen contractor</div>
          <div style="padding:14px 16px; border-bottom:1px solid #242424;">Often multiple contractors</div>

          <div style="padding:14px 16px; border-right:1px solid #242424; border-bottom:1px solid #242424;">How contractors compete</div>
          <div style="padding:14px 16px; border-right:1px solid #242424; border-bottom:1px solid #242424;">Less direct competition by design</div>
          <div style="padding:14px 16px; border-bottom:1px solid #242424;">Often a faster, crowded race</div>

          <div style="padding:14px 16px; border-right:1px solid #242424; border-bottom:1px solid #242424;">Contractor cost model</div>
          <div style="padding:14px 16px; border-right:1px solid #242424; border-bottom:1px solid #242424;">Flat $10 unlock opportunity</div>
          <div style="padding:14px 16px; border-bottom:1px solid #242424;">Pricing varies by platform and lead</div>

          <div style="padding:14px 16px; border-right:1px solid #242424;">Homeowner experience</div>
          <div style="padding:14px 16px; border-right:1px solid #242424;">Cleaner and more controlled</div>
          <div style="padding:14px 16px;">Can lead to more contractor outreach</div>
        </div>
      </div>

      <p class="mini" style="margin-top:16px; color:#c9c9c9;">
        Comparison is based on common marketplace models and general user experience patterns, which can vary by platform, service category, and market.
      </p>
    </div>
    ...
    <div class="card" style="margin-top:22px;">
      ...
      <div class="grid grid-3">
        <div class="card">
          ...
          <p class="mini">
            Homeowners choose the type of work they need and review approved contractors.
          </p>
        </div>
        <div class="card">
          <h3>2. Pick One Contractor</h3>
          <p class="mini">
            The homeowner sends one direct request to the company they actually want to hear from.
          </p>
        </div>
        <div class="card">
          <h3>3. Unlock the Opportunity</h3>
          <p class="mini">
            The contractor sees the exclusive request and can unlock full homeowner details for $10.
          </p>
        </div>
      </div>
    </div>
...
def states_page(service: str = ""):
    ...
    for code, name in states:
        href = f"/contractors?state={quote_plus(code)}"
        if service:
            href += f"&service={quote_plus(service)}"
        cards.append(
            f"""
            <a class="state-card-link" href="{href}">
              <div class="state-card premium-state-card">
                <div class="state-badge">VF</div>
                <div class="state-meta">
                  <div class="state-overline">VaultForge Market</div>
                  <h3>{esc(name)}</h3>
                  <p class="mini">
                    Browse vetted contractors{f" for {esc(service.title())}" if service else ""} in {esc(name)}.
                  </p>
                  <span class="state-cta">Enter {esc(name)} →</span>
                </div>
              </div>
            </a>
            """
    ...
    {topbar()}
    <div class="hero">
      <div class="hero-kicker gold"><strong>Choose Your Market</strong></div>
      <h1><span class="gold">Browse Contractors</span><br><span class="red">State by State</span></h1>
      <p class="hero-copy">
        VaultForge keeps homeowners and contractors organized by market so you can browse a cleaner,
        more local directory instead of getting lost in a giant mixed list.
      </p>
      <div class="topnav">
        <a class="btn" href="/services">Browse Services</a>
        <a class="btn gold" href="/contractors{'?service=' + quote_plus(service) if service else ''}">View All Approved Contractors</a>
      </div>
    </div>

    <div class="premium-panel" style="margin-bottom:22px;">
      <div class="premium-title">Why browse by state?</div>
      <div class="premium-sub">
        Homeowners see contractors serving their area. Contractors stay positioned inside the states they actually work.
        That keeps VaultForge more local, more organized, and more useful as the directory grows.
      </div>
    </div>
    ...
    """
    return layout("Browse by State", body)


@app.get("/contractors", response_class=HTMLResponse)
def contractors(service: str = "", state: str = ""):
    rows = fetch_contractors(include_unapproved=False)
    if state:
        rows = [row for row in rows if normalize_service(row.get("state")) == normalize_service(state)]
    if service:
    ...
    heading_parts = ["Approved Contractors"]
    if state:
        heading_parts.append(state.upper())
    if service:
        heading_parts.append(service.title())
    heading = " - ".join(heading_parts)

    browse_state_note = ""
    if state:
        browse_state_note = f'<div class="notice"><strong>State filter:</strong> Showing approved contractors in {esc(state.upper())}.</div>'
    ...
    <div class="hero">
      ...
      <div class="topnav">
        <a class="btn secondary" href="/states{'?service=' + quote_plus(service) if service else ''}">Choose Another State</a>
        <a class="btn" href="/services">Browse Services</a>
      </div>
    </div>
    {browse_state_note}
...
def join_contractor_form():
    ...
    <div class="hero">
      <div class="hero-kicker gold"><strong>Built for legitimate contractors who want cleaner opportunities.</strong></div>
      <h1><span class="gold">Join VaultForge</span><br><span class="red">Get seen. Get chosen. Unlock opportunity.</span></h1>
      <p class="hero-copy">
        VaultForge is not a shared-lead pile-on. Homeowners choose one contractor, not five.
        Approved contractors can unlock real homeowner opportunities for a flat $10.
      </p>
      <div class="topnav">
        <a class="btn gold" href="#contractor-application">Apply Now</a>
        <a class="btn secondary" href="/contractors">View Contractor Directory</a>
      </div>
    </div>

    <div class="grid grid-3">
      <div class="card">
        {vetted_badge_html("VaultForge Vetted")}
        <h3>Approved Contractor Network</h3>
        <p class="mini">
          Only reviewed and approved contractors are shown to homeowners. That protects trust and helps
          keep the platform cleaner for legitimate businesses.
        </p>
      </div>
      <div class="card">
        <h3>No Shared Lead Chaos</h3>
        <p class="mini">
          Homeowners select one contractor. That means less competition, less wasted time, and fewer
          race-to-the-bottom conversations.
        </p>
      </div>
      <div class="card">
        <h3>Flat $10 Unlock</h3>
        <p class="mini">
          Contractors join free. When a homeowner chooses you, you can unlock the full request for a
          flat $10 access fee.
        </p>
      </div>
    </div>
    ...
    <div class="card" style="margin-top:22px;">
      <h2 class="section-title">Why Contractors Like VaultForge</h2>
      <div class="grid grid-2">
        <div class="card">
          <h3 class="gold">VaultForge</h3>
          <ul class="clean-list mini">
            <li>Homeowner chooses one contractor</li>
            <li>No bidding war environment</li>
            <li>Cleaner, more direct homeowner requests</li>
            <li>Join free and unlock for a flat $10</li>
            <li>Approved contractor directory with a vetted badge</li>
          </ul>
        </div>
        <div class="card">
          <h3>Typical lead platforms</h3>
          <ul class="clean-list mini">
            <li>Requests are often shared with multiple contractors</li>
            <li>Higher pressure, faster competition</li>
            <li>Lead cost can be inconsistent</li>
            <li>More time spent chasing the same homeowner</li>
            <li>Less control over how the opportunity was distributed</li>
          </ul>
        </div>
      </div>
      <p class="mini" style="margin-top:14px; color:#d4af37;">
        VaultForge is designed for contractors who want a cleaner shot at real homeowner work.
      </p>
    </div>
    ...
    <div class="card" style="margin-top:22px;">
      ...
      <div class="grid grid-3">
        <div class="card">
          <h3>1. Apply</h3>
          <p class="mini">Submit your company information, service trades, and service area.</p>
        </div>
        <div class="card">
          <h3>2. Get Approved</h3>
          <p class="mini">Approved contractors appear in the homeowner directory with a vetted badge.</p>
        </div>
        <div class="card">
          <h3>3. Unlock Requests</h3>
          <p class="mini">When a homeowner chooses you, unlock the opportunity for a flat $10 fee.</p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-2" style="margin-top:22px;">
      <div class="card">
        <h2>What helps you get approved</h2>
        <ul class="clean-list mini">
          <li>Clear and professional business information</li>
          <li>Legitimate operation and presentation</li>
          <li>Licensing where required for the trade and area served</li>
          <li>Insurance coverage</li>
          <li>Trades and service areas that match homeowner demand</li>
        </ul>
      </div>
      <div class="card">
        <h2>Good fit for VaultForge</h2>
        <ul class="clean-list mini">
          <li>You want direct homeowner opportunities</li>
          <li>You do not want to fight through shared lead spam</li>
          <li>You want a stronger trust signal in front of homeowners</li>
          <li>You prefer simple pricing and cleaner flow</li>
        </ul>
      </div>
    </div>
    ...
    <div class="card" style="margin-top:22px;">
      ...
      <div class="grid grid-2">
        <div class="card">
          ...
          <p class="mini">No. Contractors join free. The $10 fee only applies when you choose to unlock a homeowner request.</p>
        </div>
        <div class="card">
          <h3>Are leads shared?</h3>
          <p class="mini">VaultForge is built around one homeowner choosing one contractor, which helps avoid the normal pile-on effect.</p>
        </div>
        <div class="card">
          <h3>How do homeowners find me?</h3>
          <p class="mini">Approved contractors appear in the service directory by trade, with trust-focused presentation and vetted positioning.</p>
        </div>
        <div class="card">
          <h3>What happens after I apply?</h3>
          <p class="mini">Your application is reviewed. If approved, you are added to the directory and can begin receiving homeowner opportunities.</p>
        </div>
      </div>
    </div>

    <div class="card" id="contractor-application" style="margin-top:22px;">
      <h2 class="section-title">Contractor Application</h2>
      <p class="mini">Complete the form below to apply for the VaultForge vetted contractor directory.</p>
      <form method="post" action="/join-contractor">
        ...
        <label>Select Your Trades</label>
        <div class="inline-note">Choose every service you want to appear under.</div>
        <div class="checkbox-grid">
          <label class="checkbox-item"><input type="checkbox" name="service" value="roofing"> Roofing</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="siding"> Siding</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="gutters"> Gutters</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="painting"> Painting</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="flooring"> Flooring</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="plumbing"> Plumbing</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="electrical"> Electrical</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="hvac"> HVAC</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="landscaping"> Landscaping</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="remodeling"> Remodeling</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="fencing"> Fencing</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="decking"> Decking</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="tree cutting"> Tree Cutting</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="land clearing"> Land Clearing</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="grading"> Grading</label>
          <label class="checkbox-item"><input type="checkbox" name="service" value="septic tank"> Septic Tank</label>
        </div>
        ...
        <div style="margin-top:20px; padding:15px; border:1px solid #444; border-radius:10px;">
          <label style="font-weight:bold; display:flex; align-items:flex-start; gap:10px;">
            <input type="checkbox" name="disclaimer" required style="margin-top:2px; width:auto;">
            <span>Before submitting, I confirm that I am <strong>licensed where applicable</strong> for my trade and <strong style="color:#d4af37;">fully insured</strong>.</span>
          </label>
        </div>

        <div class="topnav" style="margin-top:18px;">
          <button class="btn gold" type="submit">Submit Contractor Application</button>
          <a class="btn secondary" href="/contractors">Back to Contractor Directory</a>
        </div>
      </form>
    </div>
    """
...
async def join_contractor_submit(request: Request):
    ...
    selected_services = form.getlist("service") or form.getlist("services")
    selected_services = [str(s).strip().lower() for s in selected_services if str(s).strip()]
...
def request_form(contractor_id: str):
    ...
    <div class="hero">
      ...
      <div class="notice" style="margin-top:16px;">
        This request is locked to <strong>{esc(state)}</strong>. Your request will stay with this contractor's state listing.
      </div>
    </div>
    ...
    <div class="card">
      <form method="post" action="/request/{esc(contractor_id)}">
        ...
        <label>State</label>
        <input value="{esc(state)}" disabled>
        <div class="inline-note">This request is automatically locked to the contractor's state.</div>
...
def request_submit(
    ...
    contractor_state = str(contractor.get("state") or "").strip()
    lead_state = contractor_state or str(state or "").strip()
...
def admin_delete_contact(contact_id: str, request: Request):
    ...
    ok, error = delete_contact(contact_id)
    if not ok:
        return error_page(
            "Delete Contact Error",
            "We could not delete that contact message right now.",
            error,
        )
...
def admin_delete_lead(lead_id: str, request: Request):
    guard = admin_required(request)
    if guard:
        return guard

    ok, error = admin_hide_lead(lead_id)
    if not ok:
        return error_page(
            "Delete Lead Error",
            "We could not remove that homeowner lead from admin right now.",
            error,
        )
...
@app.post("/admin/delete-contractor/{contractor_id}")
def admin_delete_contractor(contractor_id: str, request: Request):
    guard = admin_required(request)
    if guard:
        return guard

    ok, error = delete_contractor_only(contractor_id)
    if not ok:
        return error_page(
            "Delete Contractor Error",
            "We could not delete that contractor right now.",
            error,
        )
...
def admin_dashboard(request: Request):
    ...
    leads = [lead for lead in fetch_leads() if lead.get("admin_deleted") is not True]
            ...
            for lead in contractor_leads:
                ...
                blocks.append(
                    ...
                    <div class="lead-admin-card">
                      <div style="display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-bottom:10px; align-items:flex-start;">
                        <div>
                          <strong>{esc(lead.get("owner_name") or "Homeowner Lead")}</strong>
                        </div>
                        <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                          {unlocked_badge}
                          <form method="post" action="/admin/delete-lead/{esc(lead.get('id'))}" onsubmit="return confirm('Remove this homeowner lead from admin only? It will stay on the contractor dashboard until the contractor deletes it.');" style="margin:0;">
                            <button class="btn secondary" type="submit">Delete Lead</button>
                          </form>
                        </div>
                      </div>
            ...
            <div class="contractor-admin-card">
              <div class="contractor-admin-head">
                ...
                <div class="contractor-admin-actions">
                  ...
                  <form method="post" action="/admin/delete-contractor/{cid}" onsubmit="return confirm('Delete this contractor only? This will not delete homeowner leads.');" style="margin:0;">
                    <button class="btn secondary" type="submit">Delete Contractor</button>
                  </form>
                </div>
              </div>
        ...
        for contact in contacts:
            contact_blocks.append(
                ...
                <div class="lead-admin-card">
                  <div style="display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-bottom:10px; align-items:flex-start;">
                    <div>
                      <strong>{esc(contact.get("name") or "Contact Message")}</strong><br>
                      <span class="pill ok" style="margin-top:8px;">Contact</span>
                    </div>
                    <div>
                      <form method="post" action="/admin/delete-contact/{esc(contact.get('id'))}" onsubmit="return confirm('Delete this contact message?');" style="margin:0;">
            ...
            blocks.append(
                ...
                <div class="lead-admin-card">
                  <div style="display:flex; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-bottom:10px; align-items:flex-start;">
                    ...
                    <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                      {unlocked_badge}
                      <form method="post" action="/admin/delete-lead/{esc(lead.get('id'))}" onsubmit="return confirm('Remove this homeowner lead from admin only?');" style="margin:0;">
                        <button class="btn secondary" type="submit">Delete Lead</button>
                      </form>
                    </div>
                  </div>
...
def contractor_dashboard(contractor_id: str, token: str = ""):
    ...
    for lead in leads:
        ...
        lead_cards.append(
            ...
            <div class="lead-premium-card">
              ...
              <div class="topnav">
                {action_button}
                <form method="post" action="/contractor/{quote_plus(contractor_id)}/delete-lead/{lead_id}" onsubmit="return confirm('Delete this homeowner lead everywhere? This removes it from the contractor dashboard and admin.');" style="margin:0;">
    ...
    <div class="hero premium-panel">
      ...
      {('<div class="vetted-badge subtle"><span class="badge-icon">✓</span><span>VaultForge Vetted Contractor</span></div>' if contractor.get("approved") is True else '')}
...
@app.post("/contractor/{contractor_id}/delete-lead/{lead_id}")
def contractor_delete_lead_route(contractor_id: str, lead_id: str, token: str = Form("")):
    contractor = fetch_contractor_by_id(contractor_id)
    if not contractor:
        return HTMLResponse("Contractor not found.", status_code=404)
    ...
    ok, error = contractor_delete_lead_record(lead_id, contractor_id)
    if not ok:
        return error_page(
            "Delete Lead Error",
            "We could not delete that homeowner lead right now.",
            error,
        )

    return RedirectResponse(url=contractor_dashboard_url(contractor_id), status_code=303)
