...
def admin_login_page(error: str = "") -> HTMLResponse:
    error_html = f'<p class="error">{safe(error)}</p>' if error else ""
    return html_page(
        "Admin Login",
        f"""
        <div class="topbar">
...
def render_join_contractor_page(error: str = "") -> HTMLResponse:
    error_html = f'<p class="error">{safe(error)}</p>' if error else ""
    return html_page(
        ...
        <div class="card">
            ...
            <form method="post" action="/join-contractor">
                ...
                <div class="checkbox-row">
                    <input type="checkbox" id="disclaimer_accepted" name="disclaimer_accepted" value="yes" required />
                    <label for="disclaimer_accepted">
                        I understand LeadForge reviews contractor applications before approval, listing is not guaranteed,
                        and homeowner opportunities are accessed through the platform terms and pricing.
                    </label>
                </div>
...
def join_contractor_form():
    return render_join_contractor_page()
...
def join_contractor_submit(
    ...
    if disclaimer_accepted != "yes":
        return render_join_contractor_page("You must accept the disclaimer to continue.")
    ...
    try:
        ...
        return render_join_contractor_page(f"Application failed to save. Details: {e}")
