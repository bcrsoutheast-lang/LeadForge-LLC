...
def copy_button(text: str, label: str = "Copy"):
    safe_text = js_attr(text)
    safe_label = esc(label)
    return f'<button type="button" class="btn secondary copy-btn" data-copy="{safe_text}" onclick="copyFromButton(this)">{safe_label}</button>'


def format_full_lead_copy_line(lead: dict, contractor: dict | None = None):
    location = f"{lead.get('city') or ''}, {lead.get('state') or ''} {lead.get('zip') or ''}".strip()
    location = " ".join(location.split())
    contractor_name = ""
    if contractor:
        contractor_name = str(contractor.get("company_name") or contractor.get("full_name") or "").strip()

    parts = [
        f"Homeowner: {str(lead.get('owner_name') or '').strip()}",
        f"Phone: {str(lead.get('phone') or '').strip()}",
        f"Email: {str(lead.get('email') or '').strip()}",
        f"Service: {str(lead.get('service') or '').strip()}",
        f"Location: {location}",
        f"Project: {str(lead.get('project_details') or '').strip()}",
    ]
    if contractor_name:
        parts.insert(0, f"Contractor: {contractor_name}")
    return " | ".join([p for p in parts if p and not p.endswith(": ")])


def admin_lead_card_html(lead: dict, contractor: dict | None = None):
    ...
    short_copy = format_copy_line(lead)
    full_copy = format_full_lead_copy_line(lead, contractor)
    phone = str(lead.get("phone") or "").strip()
    email = str(lead.get("email") or "").strip()
    text_href = f"sms:{phone}" if phone else "#"
    email_href = f"mailto:{email}" if email else "#"

    quick_actions = []
    if phone:
        quick_actions.append(f'<a class="btn secondary" href="{esc(text_href)}">Text</a>')
        quick_actions.append(copy_button(phone, "Copy Phone"))
    if email:
        quick_actions.append(f'<a class="btn secondary" href="{esc(email_href)}">Email</a>')
        quick_actions.append(copy_button(email, "Copy Email"))
    quick_actions.append(copy_button(short_copy, "Copy Short"))
    quick_actions.append(copy_button(full_copy, "Copy Full"))
    ...
    <div class="lead-admin-card">
      ...
      <div style="margin-top:12px;">
        <strong>Quick Text Line:</strong>
        <div class="copy-box">{esc(short_copy)}</div>
      </div>

      <div style="margin-top:12px;">
        <strong>Full Copy Line:</strong>
        <div class="copy-box">{esc(full_copy)}</div>
      </div>
...
def admin_dashboard(request: Request):
    ...
    contractors = sorted(
        contractors,
        key=lambda c: (
            0 if c.get("approved") is not True else 1,
            -(len(leads_by_contractor.get(str(c.get("id") or "").strip(), []))),
            str(c.get("created_at") or ""),
        ),
    )

    total_contractors = len(contractors)
    approved_count = sum(1 for c in contractors if c.get("approved") is True)
    pending_count = total_contractors - approved_count
    unlocked_count = sum(1 for lead in leads if lead.get("unlocked") is True)
    locked_count = len(leads) - unlocked_count
    ...
    for contractor in contractors:
        ...
        short_dashboard_url = BASE_URL + contractor_dashboard_url(contractor_id_raw)

        if contractor_leads:
            lead_html = "".join(admin_lead_card_html(lead, contractor) for lead in contractor_leads)
        else:
            lead_html = """
            <div class="lead-admin-card">
        ...
        contractor_cards.append(
            ...
            <div class="contractor-admin-card">
              <div class="contractor-admin-head">
                <div class="contractor-admin-meta" style="flex:1; min-width:280px;">
                  <h2 style="margin-bottom:6px;">{esc(contractor_name)}</h2>
                  <div>{approved_badge}</div>

                  <div class="field-grid" style="margin-top:14px;">
                    <div class="mini"><strong>Name:</strong><br>{esc(contractor.get("full_name"))}</div>
                    <div class="mini"><strong>Phone:</strong><br>{esc(contractor.get("phone"))}</div>
                    <div class="mini"><strong>Email:</strong><br>{esc(contractor.get("email"))}</div>
                    <div class="mini"><strong>Service:</strong><br>{esc(contractor.get("service") or contractor.get("services"))}</div>
                    <div class="mini"><strong>Area:</strong><br>{esc(contractor.get("city"))}, {esc(contractor.get("state"))} {esc(contractor.get("zip"))}</div>
                    <div class="mini"><strong>Created:</strong><br>{esc(contractor.get("created_at"))}</div>
                  </div>

                  <div class="subtle-label">Protected dashboard link</div>
                  <div class="copy-box link-box">{esc(short_dashboard_url)}</div>
                  <div class="action-row">
                    {copy_button(short_dashboard_url, "Copy Link")}
                    {copy_button(contractor.get("phone") or "", "Copy Phone")}
                    {copy_button(contractor.get("email") or "", "Copy Email")}
                  </div>
                </div>
    ...
    orphan_html = ""
    if orphan_leads:
        orphan_blocks = "".join(admin_lead_card_html(lead, None) for lead in orphan_leads)
        orphan_html = f"""
    ...
    <div class="hero">
      ...
      <p class="muted">
        Clean operations view for approvals, contractor dashboard links, and homeowner lead copy lines.
      </p>

      <div class="stat-grid">
        <div class="stat-card">
          <div class="subtle-label">Total Contractors</div>
          <div class="stat-number">{total_contractors}</div>
        </div>
        <div class="stat-card">
          <div class="subtle-label">Approved</div>
          <div class="stat-number">{approved_count}</div>
        </div>
        <div class="stat-card">
          <div class="subtle-label">Pending</div>
          <div class="stat-number">{pending_count}</div>
        </div>
        <div class="stat-card">
          <div class="subtle-label">Total Leads</div>
          <div class="stat-number">{len(leads)}</div>
        </div>
        <div class="stat-card">
          <div class="subtle-label">Unlocked</div>
          <div class="stat-number">{unlocked_count}</div>
        </div>
        <div class="stat-card">
          <div class="subtle-label">Locked</div>
...
def contractor_dashboard(contractor_id: str, token: str = ""):
    ...
    for lead in leads:
        ...
        if unlocked:
            ...
            """
            action_button = f'<a class="btn gold" href="/unlock/{lead_id}?token={quote_plus(token)}">Unlock For $10</a>'
    ...
    <div class="hero">
      ...
      <p class="muted">This dashboard is now protected by a private access token link.</p>
    </div>
