...
BASE_URL = (os.getenv("BASE_URL") or "https://leadforge-llc.onrender.com").strip()
...
def unlock_checkout(lead_id: str):
    if not STRIPE_SECRET_KEY or not STRIPE_PRICE_ID:
        return error_page(
            ...
            "Stripe secret key or price ID is missing.",
            "Check STRIPE_SECRET_KEY and STRIPE_PRICE_ID in Render environment."
        )
...
def stripe_success(session_id: str = ""):
    ...
    try:
        ...
        if payment_status == "paid" and lead_id:
            ...
            <div class="card">
              <h2>Unlocked Lead</h2>
              <p><strong>Name:</strong> {esc(lead.get("owner_name") if lead else "")}</p>
              <p><strong>Phone:</strong> {esc(lead.get("phone") if lead else "")}</p>
              <p><strong>Email:</strong> {esc(lead.get("email") if lead else "")}</p>
              <p><strong>City:</strong> {esc(lead.get("city") if lead else "")}</p>
              <p><strong>State:</strong> {esc(lead.get("state") if lead else "")}</p>
              <p><strong>ZIP:</strong> {esc(lead.get("zip") if lead else "")}</p>
              <p><strong>Service:</strong> {esc(lead.get("service") if lead else "")}</p>
              <p><strong>Project:</strong> {esc(lead.get("project_details") if lead else "")}</p>
            </div>
        ...
        return error_page(
            ...
            "Stripe returned a session, but payment was not marked as paid.",
            f"payment_status={payment_status}, lead_id={lead_id}, contractor_id={contractor_id}",
        )
    except Exception as e:
        return error_page(
            "Stripe Success Error",
            "Payment likely went through, but the app failed while finishing the unlock step.",
            str(e),
        )
