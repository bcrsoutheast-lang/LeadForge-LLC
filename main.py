...
def create_lead(lead: LeadCreate):
    ...
    if contractor.get("status") != "approved":
        raise HTTPException(status_code=400, detail="Contractor is not approved")
    ...
    existing_active = duplicate_check.data or []

    if existing_active:
        raise HTTPException(
            status_code=400,
            detail="You already have an active request in the system",
        )
